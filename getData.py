...
class CvsFileMnist(object):
    ...
    def manupulations(n_classes):
        ...
        for i in xrange(len(train_data)): 
            train_x.append(train_data[i][1:]), train_y.append(int(train_data[i][0]))
        for i in xrange(len(test_data)): 
            test_x.append(test_data[i][1:]), test_y.append(int(test_data[i][0]))      
        train_y = ActFunction.one_hot_encoded(train_y, n_classes)
        test_y = ActFunction.one_hot_encoded(test_y, n_classes)
        
        print 'we test this', len(train_y),'-', len(train_y[0])
        ...
        return {'train_x': np.asarray(train_x)/normalizer, 'train_y': train_y, 
